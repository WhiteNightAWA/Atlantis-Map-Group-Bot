...
class on_raw_reaction_add(core):
    ...
    async def on_raw_reaction_add(self, payload):
        if str(payload.emoji) in ["☑️", "🇽", "🆗", "💬", "#️⃣", "👍"] and not payload.member.bot:
            all_data = await get_all()
            guild = self.client.get_guild(858012518651002971)
            if payload.channel_id == 865180618634821632:
                        ...
                        if str(payload.emoji) == "👍":
                            ...
                            if str(newPayload.emoji) == "✅":
                                ...
                                await channel.send(embed=discord.Embed(title="材料已準備好\n請自備包包至/to !ARTC! 3F找管理員領取",
                                    ...
                                    name="材料:	", value=embed.fields[0].value))
                                ...
                                await message.add_reaction("💬")
                                await message.add_reaction("#️⃣")
            elif payload.channel_id == 865625754799833119:
                if str(payload.message_id) in all_data["channels"]:
                    if all_data["channels"][str(payload.message_id)]["stage"] == 0:
                        ...
                        if str(payload.emoji) == "☑️":
                            ...
                            await as_msg.edit(
                                embed=discord.Embed(title="頻道申請通過", description=f"處理人: <@!{payload.member.id}>",
                                                    color=discord.Colour.green()).set_author(name=user,
                                                                                             icon_url=user.avatar_url))
                            await channel.send(content=f"<@!{user.id}>",
                                               embed=discord.Embed(title="頻道申請通過",
                                                                   description=f"處理人: <@!{payload.member.id}>",
                                                                   color=discord.Colour.green()))
                            ...
                            await reson.delete()
                            if reson.content == "-":
                                await as_msg.remove_reaction("🇽", payload.member)
                            else:
                                await message.edit(content=f"<@!{user.id}>",
                                                   embed=discord.Embed(title="頻道申請被拒絕",
                                                                       description=f"處理人: <@!{payload.member.id}>",
                                                                       color=discord.Colour.red()).set_author(name=user,
                                                                                                              icon_url=user.avatar_url).add_field(
                                                       name="理由:", value=f"```{reson.content}```"))
                                await as_msg.edit(embed=discord.Embed(title="頻道申請被拒絕",
                                                                      description=f"處理人: <@!{payload.member.id}>",
                                                                      color=discord.Colour.red()).set_author(name=user,
                                                                                                             icon_url=user.avatar_url).add_field(
                                    name="理由:", value=f"```{reson.content}```"))
                                all_data["channels"].pop(str(payload.message_id), None)
                                await put_all(all_data)
