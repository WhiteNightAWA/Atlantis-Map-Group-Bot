		...
		if ctx.channel.id not in not_channel:
			...
			if str(ctx.channel.id) in data["user"]:
				if data['user'][str(ctx.channel.id)]["edit"]:
					...
					bags = ceil(total/54)
					msg = msg + f"\n總計{total}組\n需要{bags}個鑽製包包```"
					await ctx.send(embed=discord.Embed(title=f"`{data['name']}`目前申請了的材料有:",
		...
		if ctx.channel.id not in not_channel:
			...
			if str(ctx.channel.id) in all_data["user"]:
				if all_data['user'][str(ctx.channel.id)]["edit"]:
					...
					bags = ceil(total/54)
					awa = awa + f"\n總計{total}組\n需要{bags}個鑽製包包```"
					send_msg = await ctx.send(embed=discord.Embed(title="確定遞交申請?",
		...
		if ctx.channel.id not in not_channel:
			...
			if str(ctx.channel.id) in all_data["user"]:
				if not all_data['user'][str(ctx.channel.id)]["edit"]:
					...
					if message.content == "確定":
						data["edit"] = True
						send_mg = await self.client.get_channel(865180618634821632).fetch_message(all_data["user"][str(ctx.channel.id)]["message_id"])
						await send_mg.edit(
							embed=discord.Embed(title=f"此申請(`{data['name']}`)已被撤回",
						...
						all_data["data"][str(send_mg.id)]["stage"] = 0
						all_data["wait"].append(send_mg.id)
						all_data["user"][str(ctx.channel.id)] = data
						res = await put_all(all_data)
						await ctx.send(
